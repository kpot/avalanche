...
const char *get_opencl_error_string(const int error) {
    switch (error) {
...
cl::Device get_device_from_queue(const cl::CommandQueue &queue) {
    ...
    queue.getInfo(CL_QUEUE_DEVICE, &device_id);
    return cl::Device(device_id, true);
}

cl::Context get_context_from_queue(const cl::CommandQueue &queue) {
    cl_context context;
    queue.getInfo(CL_QUEUE_CONTEXT, &context);
    return cl::Context(context, true);
}
